		...
		struct device_attribute *attr, char *buf)
{
	...
	defined(CONFIG_MACH_A5_EUR_OPEN) || \
	defined(CONFIG_MACH_A5_CHN_OPEN) || \
	defined(CONFIG_MACH_A5_CHN_ZH) || \
	defined(CONFIG_MACH_A5_CHN_ZT) || \
	defined(CONFIG_MACH_A5_CHN_CTC) || \
	defined(CONFIG_MACH_A53G_EUR_OPEN) || \
	defined(CONFIG_MACH_E7_EUR_OPEN)
...
#if defined(CONFIG_MACH_KLEOS_EUR)
	pr_info("%s: Disable request card detection\n",__func__);
#else
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
