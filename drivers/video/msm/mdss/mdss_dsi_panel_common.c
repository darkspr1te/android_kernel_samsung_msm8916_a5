...
#define SAMSUNG_DISP_SEND_CMD /* oled panel */
#endif
...
#if defined(CONFIG_FB_MSM_MDSS_CHECK_LCDID)
static unsigned int mdss_dsi_read_manufacture_id ( struct mdss_dsi_ctrl_pdata *ctrl )
{
	char cmd0 = 0xDA;
	char cmd1 = 0x00;
	char rbuf[3] = { 0x00, 0x00, 0x00 };
	unsigned int lcd_id = 0x0;

	dcs_read_cmd.dchdr.dtype = DTYPE_GEN_READ1;

	cmd0 = 0xDA;
	mdss_dsi_panel_cmd_read ( ctrl, cmd0, cmd1, NULL, rbuf, 1 );
	lcd_id = (lcd_id << 8) | ctrl->rx_buf.data[0];
	pr_info ( "%s-LCDID1(0x%x)\n", __func__, ctrl->rx_buf.data[0]);

	cmd0 = 0xDB;
	mdss_dsi_panel_cmd_read ( ctrl, cmd0, cmd1, NULL, rbuf, 1 );
	lcd_id = (lcd_id << 8) | ctrl->rx_buf.data[0];
	pr_info ( "%s-LCDID2(0x%x)\n", __func__, ctrl->rx_buf.data[0]);

	cmd0 = 0xDC;
	mdss_dsi_panel_cmd_read ( ctrl, cmd0, cmd1, NULL, rbuf, 1 );
	lcd_id = (lcd_id << 8) | ctrl->rx_buf.data[0];
	pr_info ( "%s-LCDID3(0x%x)\n", __func__, ctrl->rx_buf.data[0]);
...
#endif
	enum mdss_cmd_list cmd, unsigned char lock)
{
...
#endif
#ifdef SMART_DIMMING
	struct mdss_samsung_driver_data *s_pdata = NULL;
	if (!pdata) {
	...
	switch (cmd) {
		...
		case PANEL_DISPLAY_ON_SEQ:
			cmd_desc.cmds = ctrl->cmd_list.disp_on_seq.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.disp_on_seq.cmd_cnt;
			break;
		case PANEL_DISPLAY_OFF_SEQ:
			cmd_desc.cmds = ctrl->cmd_list.disp_off_seq.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.disp_off_seq.cmd_cnt;
			break;
		case PANEL_DISPLAY_ON:
			cmd_desc.cmds = ctrl->cmd_list.disp_on_cmd.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.disp_on_cmd.cmd_cnt;
			break;
		case PANEL_DISPLAY_OFF:
			cmd_desc.cmds = ctrl->cmd_list.disp_off_cmd.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.disp_off_cmd.cmd_cnt;
			break;
		case PANEL_BRIGHT_CTRL:
			/* TODO: remove "break;", if the smart dimming working fine */
			break;
			cmd_desc.cmds = brightness_packet;

			if (s_pdata->dstat.bright_level)
				s_pdata->dstat.recent_bright_level = s_pdata->dstat.bright_level;
			cmd_desc.cmd_cnt = make_brightcontrol_set(pdata, s_pdata->dstat.bright_level);
		...
		case PANEL_MTP_ENABLE:
			cmd_desc.cmds = ctrl->cmd_list.mtp_enable_cmd.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.mtp_enable_cmd.cmd_cnt;
			break;
		case PANEL_MTP_DISABLE:
			cmd_desc.cmds = ctrl->cmd_list.mtp_disable_cmd.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.mtp_disable_cmd.cmd_cnt;
			break;
		case PANEL_ACL_OFF:
			cmd_desc.cmds = ctrl->cmd_list.acl_off_cmd.cmds;
			cmd_desc.cmd_cnt = ctrl->cmd_list.acl_off_cmd.cmd_cnt;
		    break;
#endif
			default:
...
static int update_bright_packet(struct mdss_panel_data *pdata, int cmd_count, struct dsi_panel_cmds *cmd_set)
{
	...
	for (i = 0; i < cmd_set->cmd_cnt; i++) {
		brightness_cmd->cmds[cmd_count].payload = cmd_set->cmds[i].payload;
		brightness_cmd->cmds[cmd_count].dchdr.dlen = cmd_set->cmds[i].dchdr.dlen;
		brightness_cmd->cmds[cmd_count].dchdr.dtype = cmd_set->cmds[i].dchdr.dtype;
		brightness_cmd->cmds[cmd_count].dchdr.wait = cmd_set->cmds[i].dchdr.wait;
		cmd_count++;
	}
							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
#ifdef SMART_DIMMING
		mdss_disp_send_cmd(pdata, PANEL_MTP_ENABLE, true);
		s_pdata->dstat.bright_level = bl_level;
		mdss_disp_send_cmd(pdata, PANEL_BRIGHT_CTRL, true);
		mdss_disp_send_cmd(pdata, PANEL_MTP_DISABLE, true);
#else
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	lcd_id = mdss_dsi_read_manufacture_id ( ctrl );
	pr_info ( "%s-lcd_id(0x%x)\n", __func__, lcd_id );

	if( lcd_id == pinfo->mipi.panel_id )
		msd.manufacture_id = lcd_id;
	else
		msd.manufacture_id = 0;
#endif
...
#if defined(CONFIG_FB_MSM_MDSS_CHECK_LCDID)
	rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-id", &tmp);
	pinfo->mipi.panel_id = (!rc ? tmp : 0x555555);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (power) {
		...
		if(msd.ctrl_pdata == NULL){
			...
			mdss_dsi_panel_bklt_dcs(msd.ctrl_pdata, 255);
		}
	} else {
...
#if 1 //defined(CONFIG_GET_LCD_ATTACHED)
