...
static struct dsi_cmd elvss_hbm_temperature_cmds;
#endif
...
#endif
static struct dsi_cmd get_elvss_caps_set(int cd_level)
{
...
static struct dsi_cmd get_elvss_control_set(int cd_idx, int cd_level)
{
...
#endif
	/*elvss*/
	elvss_control = get_elvss_control_set(cd_idx, cd_level);
	cmd_count = update_bright_packet(cmd_count, &elvss_control);

	elvss_caps_set = get_elvss_caps_set(cd_level);
...
/*	struct msm_fb_data_type *mfd = msd.mfd;*/
	...
	switch (cmd) {
		...
		case HBM_ELVSS_TEMPER:
			if (msd.dstat.temper_need_update) {
				msd.dstat.temper_need_update = 0;
				elvss_hbm_temperature_cmds.cmd_desc[1].payload[1] =
					msd.dstat.temperature_value;
			}
			cmd_desc = elvss_hbm_temperature_cmds.cmd_desc;
			cmd_size = elvss_hbm_temperature_cmds.num_of_cmds;
			break;
#endif
		default:
...
#if defined(CONFIG_FB_MSM_MIPI_EA8061V_PANEL)
	mdss_dsi_panel_cmd_read(msd.ctrl_pdata, 0xC8, 0x00, NULL, hbm_buffer, 57);
	memcpy(msd.sdimconf->mtp_buffer, hbm_buffer, 33);
#else
...
#if defined(HBM_RE)
#if defined(CONFIG_FB_MSM_MIPI_EA8061V_PANEL)
	memcpy(&hbm_gamma_cmds_list.cmd_desc[0].payload[1], hbm_buffer + 33, 6);
	memcpy(&hbm_etc_cmds_list.cmd_desc[4].payload[1], hbm_buffer + 39, 1);
	memcpy(&hbm_gamma_cmds_list.cmd_desc[0].payload[7], hbm_buffer + 42, 15);
	mipi_samsung_read_nv_mem(pdata, &nv_mtp_hbm3_read_cmds, hbm_buffer);
	memcpy(&hbm_off_cmds.cmd_desc[3].payload[1], hbm_buffer+3, 1);
#else
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_samsung_parse_panel_cmd(np, &elvss_hbm_temperature_cmds,
		"samsung,panel-hbm-elvss-temper-cmds");
#endif
	...
	rc = of_property_read_u32(np, "qcom,mdss-dsi-booting-brightness", &tmp);
	msd.dstat.bright_level = (!rc ? tmp : 0);
